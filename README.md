...
$ vagrant ssh
```
You will now be connected inside of the virtual OS. Run `sudo less /etc/rancher/k3s/k3s.yaml` to print out the contents of the file. You should see output similar to the one that I've shown below. Note that the output below is just for your reference: every configuration is unique and you should _NOT_ copy the output I have below.

Copy the contents from the output issued from your own command into your clipboard -- we will be pasting it somewhere soon!
```bash
$ sudo less /etc/rancher/k3s/k3s.yaml
...
### Steps
1. `kubectl apply -f deployment/db-configmap.yaml` - Set up environment variables for the db pod
2. `kubectl apply -f deployment/kafka-configmap.yaml` - Set up environment variables for kafka
3. `kubectl apply -f deployment/db-secret.yaml` - Set up secrets for the db pod
...
7. `kubectl apply -f deployment/kafka.yaml` - Set up the service and deployment for the zookeeper and the kafka-broker
8. `kubectl apply -f deployment/producer-locations-grpc.yaml` - Set up the service and deployment for the Location producer
9. `kubectl apply -f deployment/consumer-locations-kafka.yaml` - Set up the service and deployment for the Location consumer
...
You can check the architecture diagram in Docs/architecture_design.png

## API's
- check the Docs/openapi.yaml for the Person/Connection RESTful API
- read the Docs/grpc.txt how to test the gRPC endpoints for Locations
